...
import BASEAPI from "@/lib/api/base/request";
    ...
    Object.keys(handler?.data || {}).map(async (key1) => {
      return await Promise.all(
        Object.keys(handler?.data?.[key1] || {}).map(async (key2) => {
          ...
          if (key2 === "custom_api") {
            const data = handler?.data?.[key1];
            const { custom_api = "" } = data;
            const res = await BASEAPI.get(custom_api);
            const dataHandler = dataFormatter.deserialize(res);
            data.custom_data = clean(dataHandler);
            clean(data);
          }
        })
      );
